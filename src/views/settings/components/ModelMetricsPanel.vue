...
import {
  ...
  fetchModelMetricsDetail,
  fetchModelCompare,
  fetchModelHealthOverview,
  fetchModelVersionOptions,
...
const viewMode = ref<'dashboard' | 'compare'>(
  props.fixedMode === 'compare' ? 'compare' : 'dashboard',
)
  ...
  return {
    ...
    series: [
      ...
      { name: '物理合规性', type: 'line', data: hist.map((h) => h.compliance_score), smooth: true },
      {
        name: '综合评分',
        type: 'line',
        data: hist.map((h) => h.overall_score),
        smooth: true,
    ...
    series: [
      {
        ...
        data: [
          {
            name: compare.value.current.version,
            value: dims.map((d) => compare.value!.current.scores[d]),
          },
          {
            name: compare.value.previous.version,
            value: dims.map((d) => compare.value!.previous.scores[d]),
          },
        ],
      },
    ],
  ...
  try {
    ...
    if (!previousVersion.value)
      previousVersion.value =
        versionOptions.value[1]?.version ?? versionOptions.value[0]?.version ?? ''
...
<template>
  <div v-loading="loading" class="gateai-panel">
    ...
    <template
      v-else-if="
        compare && (fixedMode === 'compare' || (fixedMode === 'both' && viewMode === 'compare'))
      "
    >
