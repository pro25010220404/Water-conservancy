...
const weightForm = ref({ power_weight: 0.40, safety_weight: 0.35, ecology_weight: 0.25 })
const weightLoading = ref(false)
const presetOptions = [
  { label: '均衡方案', power: 0.40, safety: 0.35, ecology: 0.25 },
  { label: '发电优先', power: 0.60, safety: 0.25, ecology: 0.15 },
  { label: '安全优先', power: 0.15, safety: 0.70, ecology: 0.15 },
  { label: '生态优先', power: 0.20, safety: 0.25, ecology: 0.55 },
]
...
const metricLabelMap: Record<string, string> = {
  upstream_level: '上游水位', downstream_level: '下游水位', inflow_rate: '入库流量',
  outflow_rate: '出库流量', gate_opening: '闸门开度', power_output: '发电功率',
}

// 角色
const roleOptions = [
  { label: '值班运维', value: 1 }, { label: '调度工程师', value: 2 },
  { label: '站长/管理', value: 3 }, { label: '系统管理员', value: 4 }, { label: '算法工程师', value: 5 },
]
const roleLabel = computed(() => {
  const map: Record<number, string> = {}
  roleOptions.forEach((r) => { map[r.value] = r.label })
  return map
})

// ── 6. Computed (权重合计) ──
const weightSum = computed(() =>
  +(weightForm.value.power_weight + weightForm.value.safety_weight + weightForm.value.ecology_weight).toFixed(2),
)
...
function onSliderChange(changed: string) {
  ...
  if (sumOthers === 0) {
    others.forEach((k) => { weightForm.value[k] = +(rest / others.length).toFixed(2) })
  } else {
    others.forEach((k) => { weightForm.value[k] = +((rest * weightForm.value[k]) / sumOthers).toFixed(2) })
  }
}
...
const MOCK_THRESHOLDS: ThresholdRule[] = [
  { id: 1, reservoir_id: 1, metric: 'upstream_level', warning_upper: 93.0, warning_lower: 88.0, critical_upper: 95.0, critical_lower: 86.0, debounce_seconds: 30, enabled: 1 },
  { id: 2, reservoir_id: 1, metric: 'downstream_level', warning_upper: 86.5, warning_lower: 84.0, critical_upper: 88.0, critical_lower: 83.0, debounce_seconds: 30, enabled: 1 },
  { id: 3, reservoir_id: 1, metric: 'inflow_rate', warning_upper: 500.0, warning_lower: 100.0, critical_upper: 600.0, critical_lower: 80.0, debounce_seconds: 30, enabled: 1 },
  { id: 4, reservoir_id: 1, metric: 'outflow_rate', warning_upper: 450.0, warning_lower: 50.0, critical_upper: 550.0, critical_lower: 30.0, debounce_seconds: 30, enabled: 1 },
  { id: 5, reservoir_id: 1, metric: 'gate_opening', warning_upper: 80.0, warning_lower: 10.0, critical_upper: 90.0, critical_lower: 5.0, debounce_seconds: 30, enabled: 1 },
  { id: 6, reservoir_id: 1, metric: 'power_output', warning_upper: 200.0, warning_lower: 50.0, critical_upper: 250.0, critical_lower: 30.0, debounce_seconds: 30, enabled: 0 },
]
...
const MOCK_USERS: SystemUser[] = [
  ...
  { id: 2, account: 'zhangsan', realname: '张三', role_id: 1, role_name: '值班运维人员', phone: '13800001001', is_enabled: 1, created_at: '2026-06-15 09:30:00' },
  { id: 3, account: 'lisi', realname: '李四', role_id: 2, role_name: '调度决策工程师', phone: '13800001002', is_enabled: 1, created_at: '2026-06-20 10:00:00' },
  { id: 4, account: 'wangwu', realname: '王五', role_id: 3, role_name: '站长/管理人员', phone: '13800001003', is_enabled: 1, created_at: '2026-06-22 14:00:00' },
  { id: 5, account: 'zhaoliu', realname: '赵六', role_id: 5, role_name: '算法工程师', phone: '13800001004', is_enabled: 0, created_at: '2026-06-25 11:00:00' },
  { id: 6, account: 'sunqi', realname: '孙七', role_id: 1, role_name: '值班运维人员', phone: '13800001005', is_enabled: 1, created_at: '2026-07-02 08:30:00' },
]
...
async function fetchThresholds() {
  ...
  try {
    ...
    if (res.data.code === 0 && res.data.data?.length) { thresholds.value = res.data.data; thresholdsLoading.value = false; return }
  } catch { /* fallback */ }
...
async function saveThreshold(id: number) {
  ...
  if (edit.warning_lower >= edit.warning_upper || edit.critical_lower >= edit.critical_upper) {
    ElMessage.warning('下限必须小于上限'); return
  }
  if (edit.debounce_seconds < 10 || edit.debounce_seconds > 120) {
    ElMessage.warning('防抖时间范围 10-120 秒'); return
  }
...
async function fetchWeights() {
  ...
  try {
    ...
    if (res.data.code === 0 && res.data.data) {
      ...
      weights.value = d
      weightForm.value = { power_weight: d.power_weight, safety_weight: d.safety_weight, ecology_weight: d.ecology_weight }
      weightLoading.value = false; return
    }
  } catch { /* fallback */ }
...
async function saveWeights() {
  if (!weightValid.value) { ElMessage.warning('三权重之和必须等于 1.0'); return }
...
async function fetchModels() {
  ...
  try {
    const res = await getModels({ page: modelsPage.value, keyword: modelKeyword.value || undefined })
    if (res.data.code === 0 && res.data.data.list.length) { models.value = res.data.data.list; modelsTotal.value = res.data.data.total; modelsLoading.value = false; return }
  } catch { /* fallback */ }
...
async function handleDeployModel(id: number) {
  try {
    const ids = await ElMessageBox.prompt('请输入目标边缘节点 ID（逗号分隔）', '下发模型', { confirmButtonText: '下发' })
...
async function fetchUsers() {
  ...
  try {
    ...
    if (res.data.code === 0 && res.data.data.list.length) { users.value = res.data.data.list; usersTotal.value = res.data.data.total; usersLoading.value = false; return }
  } catch { /* fallback */ }
...
function openUserDialog(mode: 'create' | 'edit', row?: SystemUser) {
  ...
  if (mode === 'edit' && row) {
    ...
    userForm.value = { account: row.account, password: '', realname: row.realname, role_id: row.role_id, phone: row.phone || '' }
  } else {
...
<template>
  <div class="page settings-page">
    <el-tabs v-model="activeTab" type="border-card" class="settings-page__tabs">

      <!-- ═══ Tab1: 告警阈值 ═══ -->
      <el-tab-pane label="告警阈值配置" name="thresholds">
        <el-table :data="thresholds" v-loading="thresholdsLoading" style="width: 100%">
          <el-table-column prop="metric" label="监控指标" min-width="140">
            <template #default="scope">
              {{ metricLabelMap[(scope.row as ThresholdRule).metric] ?? (scope.row as ThresholdRule).metric }}
            </template>
          </el-table-column>
          <el-table-column label="预警上限" width="130" align="center">
            <template #default="scope">
              <el-input-number v-if="thresholdsEditing[(scope.row as ThresholdRule).id]" v-model="thresholdsEditing[(scope.row as ThresholdRule).id].warning_upper" :step="0.1" controls-position="right" style="width:110px" />
              <span v-else>{{ (scope.row as ThresholdRule).warning_upper }}</span>
            </template>
          </el-table-column>
          <el-table-column label="预警下限" width="130" align="center">
            <template #default="scope">
              <el-input-number v-if="thresholdsEditing[(scope.row as ThresholdRule).id]" v-model="thresholdsEditing[(scope.row as ThresholdRule).id].warning_lower" :step="0.1" controls-position="right" style="width:110px" />
              <span v-else>{{ (scope.row as ThresholdRule).warning_lower }}</span>
            </template>
          </el-table-column>
          <el-table-column label="紧急上限" width="130" align="center">
            <template #default="scope">
              <el-input-number v-if="thresholdsEditing[(scope.row as ThresholdRule).id]" v-model="thresholdsEditing[(scope.row as ThresholdRule).id].critical_upper" :step="0.1" controls-position="right" style="width:110px" />
              <span v-else>{{ (scope.row as ThresholdRule).critical_upper }}</span>
            </template>
          </el-table-column>
          <el-table-column label="紧急下限" width="130" align="center">
            <template #default="scope">
              <el-input-number v-if="thresholdsEditing[(scope.row as ThresholdRule).id]" v-model="thresholdsEditing[(scope.row as ThresholdRule).id].critical_lower" :step="0.1" controls-position="right" style="width:110px" />
          ...
          <el-table-column label="启用" width="70" align="center">
            <template #default="scope">
              <el-switch v-if="thresholdsEditing[(scope.row as ThresholdRule).id]" v-model="thresholdsEditing[(scope.row as ThresholdRule).id].enabled" :active-value="1" :inactive-value="0" />
              <el-tag v-else :type="(scope.row as ThresholdRule).enabled === 1 ? 'success' : 'info'">{{ (scope.row as ThresholdRule).enabled === 1 ? '启用' : '停用' }}</el-tag>
            </template>
          </el-table-column>
            ...
            <template #default="scope">
              <template v-if="thresholdsEditing[(scope.row as ThresholdRule).id]">
                <div class="threshold-actions">
                  <el-button type="primary" size="small" :loading="saveLoadingTab1" @click="saveThreshold((scope.row as ThresholdRule).id)">保存</el-button>
                  <el-button size="small" @click="cancelEditThreshold((scope.row as ThresholdRule).id)">取消</el-button>
                </div>
              </template>
              <el-button v-else type="primary" link @click="startEditThreshold(scope.row as ThresholdRule)">编辑</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-tab-pane>
      ...
      <!-- ═══ Tab2: 多目标权重 ═══ -->
      <el-tab-pane label="多目标权重配置" name="weights">
        <el-card v-loading="weightLoading" class="settings-page__weight-card" shadow="never">
          <div class="weight-section">
            <div class="weight-presets">
              <span class="weight-label">预设方案：</span>
              <el-button v-for="p in presetOptions" :key="p.label" style="margin-right:8px" @click="applyPreset(p)">{{ p.label }}</el-button>
            </div>
            <div class="weight-sliders">
              <div class="weight-row">
                <span class="weight-row__label">发电效益</span>
                <el-slider v-model="weightForm.power_weight" :min="0" :max="1" :step="0.01" style="flex:1;margin:0 16px" @input="onSliderChange('power_weight')" />
                <span class="weight-row__value">{{ (weightForm.power_weight * 100).toFixed(0) }}%</span>
              </div>
              <div class="weight-row">
                <span class="weight-row__label">防洪安全</span>
                <el-slider v-model="weightForm.safety_weight" :min="0" :max="1" :step="0.01" style="flex:1;margin:0 16px" @input="onSliderChange('safety_weight')" />
                <span class="weight-row__value">{{ (weightForm.safety_weight * 100).toFixed(0) }}%</span>
              </div>
              <div class="weight-row">
                <span class="weight-row__label">生态流量</span>
                <el-slider v-model="weightForm.ecology_weight" :min="0" :max="1" :step="0.01" style="flex:1;margin:0 16px" @input="onSliderChange('ecology_weight')" />
            ...
            <div class="weight-summary">
              ...
              <span v-if="!weightValid" class="weight-summary--warn"><el-icon><Warning /></el-icon>三权重之和必须等于 1.0 才能保存</span>
            </div>
            <el-button type="primary" :disabled="!weightValid" :loading="saveLoadingTab2" @click="saveWeights" style="margin-top:16px">
              保存并推送至边缘端
            </el-button>
          </div>
        </el-card>
      ...
      <el-tab-pane label="模型管理" name="models">
        <div class="settings-page__toolbar">
          <el-input v-model="modelKeyword" placeholder="搜索模型名称" :prefix-icon="Search" clearable style="width:220px" @input="modelsPage=1;fetchModels()" />
          ...
          >
            ...
            <template #tip>
              <div class="upload-tip" style="font-size:12px;color:var(--color-text-secondary);margin-top:4px">
                支持 .pt / .pth / .onnx / .h5 / .pb / .zip，单文件 ≤500MB
              </div>
            </template>
          </el-upload>
        </div>
        <el-table :data="models" v-loading="modelsLoading" stripe border style="width:100%;margin-top:12px" table-layout="auto">
          ...
          <el-table-column prop="size" label="大小(MB)" width="90" />
          <el-table-column prop="deployed_nodes" label="已下发节点" width="100" />
          <el-table-column label="操作" width="240" fixed="right">
            <template #default="scope">
              <el-button v-if="(scope.row as ModelInfo).status !== 'active'" type="success" link @click="handleActivateModel((scope.row as ModelInfo).id)">激活</el-button>
              <el-button v-else type="warning" link @click="handleRollbackModel((scope.row as ModelInfo).id)">回滚</el-button>
              <el-button type="primary" link @click="handleDeployModel((scope.row as ModelInfo).id)">下发</el-button>
              <el-button v-if="(scope.row as ModelInfo).status !== 'active'" type="danger" link @click="handleDeleteModel((scope.row as ModelInfo).id)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
      ...
      <el-tab-pane label="用户管理" name="users">
        <div class="settings-page__toolbar">
          ...
          <el-button type="primary" :icon="Plus" style="margin-left:auto" @click="openUserDialog('create')">新增用户</el-button>
        </div>
        <el-table :data="users" v-loading="usersLoading" stripe border style="width:100%;margin-top:12px" table-layout="auto">
          ...
          <el-table-column label="角色" min-width="110" align="center">
            <template #default="scope">
              <el-tag type="info">{{ roleLabel[(scope.row as SystemUser).role_id] ?? (scope.row as SystemUser).role_name }}</el-tag>
            </template>
          </el-table-column>
          <el-table-column label="状态" width="80" align="center">
            <template #default="scope">
              <el-tag :type="(scope.row as SystemUser).is_enabled === 1 ? 'success' : 'danger'" effect="plain">
          ...
          <el-table-column label="操作" width="240" fixed="right" align="center">
            <template #default="scope">
              <el-button type="primary" link @click="openUserDialog('edit', scope.row as SystemUser)">编辑</el-button>
              <el-button link @click="handleResetPwd(scope.row as SystemUser)">重置</el-button>
              <el-button v-if="(scope.row as SystemUser).is_enabled === 1" type="warning" link @click="handleLock(scope.row as SystemUser)">锁定</el-button>
              <el-button v-else type="success" link @click="handleUnlock(scope.row as SystemUser)">解锁</el-button>
              <el-button type="danger" link @click="handleDelete(scope.row as SystemUser)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination v-model:current-page="usersPage" :page-size="10" :total="usersTotal" layout="total, prev, pager, next" background style="margin-top:12px;justify-content:flex-end" @current-change="fetchUsers" />

        <!-- 用户弹窗 -->
        <el-dialog v-model="userDialogVisible" :title="userDialogMode === 'create' ? '新增用户' : '编辑用户'" width="480px">
          <el-form :model="userForm" label-width="80px" :rules="userDialogMode === 'create' ? { account: FORM_RULES.account, password: FORM_RULES.password, realname: FORM_RULES.realname } : { realname: FORM_RULES.realname }">
            <el-form-item v-if="userDialogMode === 'create'" label="用户名" prop="account">
              <el-input v-model="userForm.account" placeholder="≥3位字母数字下划线" />
            </el-form-item>
            <el-form-item v-if="userDialogMode === 'create'" label="密码" prop="password">
              <el-input v-model="userForm.password" type="password" placeholder="≥8位含字母数字" show-password />
            </el-form-item>
            <el-form-item label="姓名" prop="realname">
              <el-input v-model="userForm.realname" placeholder="2-20个字符" maxlength="20" />
            </el-form-item>
            ...
            <el-form-item label="手机号">
              <el-input v-model="userForm.phone" placeholder="11位手机号" maxlength="11" />
            </el-form-item>
          </el-form>
          <template #footer>
            <el-button @click="userDialogVisible = false">取消</el-button>
            <el-button type="primary" :loading="userSubmitting" @click="submitUser">{{ userDialogMode === 'create' ? '创建' : '保存' }}</el-button>
          </template>
        </el-dialog>
      </el-tab-pane>
    </el-tabs>
...
.weight-row {
  ...
  &__label { width: 100px; flex-shrink: 0; font-size: var(--font-size-lg); color: var(--color-text); font-weight: 500; }
  &__value { width: 60px; text-align: right; font-weight: 700; font-size: var(--font-size-lg); color: var(--color-primary); }
}
...
.weight-summary {
  ...
  &--warn { display: flex; align-items: center; gap: 4px; font-size: var(--font-size-base); color: #ef4444; font-weight: 400; }
}
