  ...
  <ElCard v-loading="loading" shadow="never" class="reservoir-panel">
    <template #header>
      <div class="reservoir-panel__head">
        ...
        <span v-if="detail" class="reservoir-panel__sub">{{ detail.name }} · {{ detail.code }}</span>
      </div>
    </template>
    ...
    <template v-if="detail">
      <ElDescriptions :column="3" border size="small" class="reservoir-panel__base">
        ...
        <ElDescriptionsItem label="正常蓄水位">{{ detail.normal_water_level }} m</ElDescriptionsItem>
        <ElDescriptionsItem label="装机容量">{{ (detail.installed_capacity / 10000).toFixed(0) }} 万 kW</ElDescriptionsItem>
      ...
      <div v-if="physics" class="reservoir-panel__physics">
        <div class="reservoir-panel__physics-head">
          <h4>物理防护配置摘要</h4>
          <ElButton type="primary" link size="small" @click="router.push({ path: '/settings', query: { tab: 'physics-guard', reservoir_id: String(props.reservoirId) } })">
            管理配置 →
          </ElButton>
        </div>
        <div class="reservoir-panel__physics-bar">
          ...
          <span v-if="physics.last_sync_at" class="reservoir-panel__sync">同步 {{ physics.last_sync_at }}</span>
        </div>
      </div>
    </template>
