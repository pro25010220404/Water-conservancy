...
const replaySnapshot = computed(() => {
  let best = allData.value[0], bestDiff = Infinity
...
function startTimer() {
  ...
  replayTimer = setInterval(() => {
    ...
    if (replayTime.value >= replayTimeMax.value) { replayTime.value = replayTimeMax.value; stopTimer() }
  }, 100)
}
function stopTimer() {
...
function seek(e: Event) { replayTime.value = Number((e.target as HTMLInputElement).value) }
function jump(d: number) {
  const step = 5 * 15 * 60000
  replayTime.value = Math.min(replayTimeMax.value, Math.max(replayTimeMin.value, replayTime.value + d * step))
}
function reset() { stopTimer(); replayTime.value = allData.value[Math.floor(allData.value.length / 2)]?.time ?? Date.now() }

onMounted(() => { replayTime.value = allData.value[Math.floor(allData.value.length / 2)]?.time ?? Date.now() })
...
const chartOpt = computed(() => {
  ...
  return {
    ...
    legend: { data: metrics.map(m => m.label), textStyle: { color: '#64748b', fontSize: 13 }, top: 0 },
    grid: { left: 60, right: 40, top: 36, bottom: 60 },
    tooltip: { trigger: 'axis' },
    xAxis: { type: 'category', data: data.map(d => d.label), axisLabel: { interval: Math.floor(data.length / 6), fontSize: 11 } },
    yAxis: [
  ...
  <div class="rp">
    ...
    <div class="rp__snap">
      <div v-for="m in metrics" :key="m.key" class="rp__snap-item">
        ...
        <span class="rp__snap-val" :style="{ color: m.color }">{{ replaySnapshot?.[m.key] ?? '--' }} {{ m.unit }}</span>
      </div>
    </div>
    ...
    <div class="rp__ctrl">
      ...
      <div class="rp__speeds">
        <button v-for="s in [1,2,5,10]" :key="s" class="rp__sp" :class="{ on: replaySpeed === s }" @click="replaySpeed = s">{{ s }}x</button>
      </div>

      <div class="rp__slider">
        <input type="range" :min="replayTimeMin" :max="replayTimeMax" :value="replayTime" @input="seek" />
        <span>{{ progress }}%</span>
      </div>
    </div>
  </div>
...
.rp {
  display: flex; flex-direction: column; height: 100%; background: #f8f9fb; overflow: hidden;
  &__snap {
  ...
  &__snap-item { display: flex; flex-direction: column; align-items: center; gap: 2px; }
  &__snap-label { font-size: 12px; color: #92400e; font-weight: 500; }
  &__snap-val { font-size: 18px; font-weight: 700; font-family: 'SF Mono', monospace; }
  &__chart { flex: 1; min-height: 0; padding: 12px 16px 8px; }
  &__ctrl {
    display: flex; align-items: center; gap: 10px; padding: 12px 24px;
    background: #fff; border-top: 1px solid #eef0f2; flex-shrink: 0; flex-wrap: wrap;
  }
  &__btn {
    width: 38px; height: 38px; display: flex; align-items: center; justify-content: center;
    font-size: 18px; color: #92400e; background: #fffbeb; border: 1px solid #f59e0b; border-radius: 8px; cursor: pointer;
    &:hover { background: #fef3c7; }
    &--play { width: 46px; height: 46px; font-size: 20px; background: #f59e0b; color: #fff; border-radius: 50%; border-color: transparent;
      &:hover { background: #d97706; }
    }
  }
  &__time { min-width: 180px; font-size: 16px; font-weight: 700; font-family: 'SF Mono', monospace; color: #92400e; }
  &__speeds { display: flex; gap: 3px; }
  &__sp { padding: 5px 12px; font-size: 13px; font-weight: 600; color: #92400e; background: #fff; border: 1px solid #fcd34d; border-radius: 5px; cursor: pointer;
    &:hover { background: #fffbeb; }
    &.on { color: #fff; background: #f59e0b; border-color: #f59e0b; }
  }
  &__slider { flex: 1; min-width: 150px; display: flex; align-items: center; gap: 8px;
    input { flex: 1; height: 6px; -webkit-appearance: none; appearance: none; background: linear-gradient(90deg, #fcd34d, #f59e0b); border-radius: 3px; outline: none; cursor: pointer;
      &::-webkit-slider-thumb { -webkit-appearance: none; width: 20px; height: 20px; border-radius: 50%; background: #f59e0b; border: 3px solid #fff; box-shadow: 0 2px 6px rgba(245,158,11,.5); cursor: pointer; }
    }
    span { font-size: 12px; font-family: 'SF Mono', monospace; color: #92400e; min-width: 40px; text-align: right; }
  }
}
</style>
