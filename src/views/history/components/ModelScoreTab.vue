...
const chartOpt = computed(() => ({
  ...
  xAxis: { type: 'category', data: scoreData.value.map(d => d.time), axisLabel: { interval: Math.max(1, Math.floor(days.value / 10)), rotate: 30 } },
  yAxis: { type: 'value', min: 0, max: 1, axisLabel: { formatter: (v: number) => (v * 100).toFixed(0) } },
  dataZoom: [{ type: 'slider', bottom: 0, height: 22 }],
  series: [
    { name: '预测准确性', type: 'line', data: scoreData.value.map(d => d.prediction), smooth: true, lineStyle: { color: '#5470C6', width: 2 }, symbol: 'none' },
    { name: '决策可靠性', type: 'line', data: scoreData.value.map(d => d.decision), smooth: true, lineStyle: { color: '#91CC75', width: 2 }, symbol: 'none' },
    { name: '物理合规性', type: 'line', data: scoreData.value.map(d => d.compliance), smooth: true, lineStyle: { color: '#FAC858', width: 2 }, symbol: 'none' },
    { name: '综合评分', type: 'line', data: scoreData.value.map(d => d.overall), smooth: true, lineStyle: { color: '#999', width: 1.5, type: 'dashed' }, symbol: 'none' },
  ],
}))
  ...
  <div class="ms">
    <div class="ms__filters">
      ...
      <div class="ms__grades">
        ...
        <span v-for="g in ['S','A','B','C','D']" :key="g" class="ms__g" :class="{ on: gradeFilter === g }" :style="{ '--gc': (HEALTH_GRADE as any)[g]?.color }" @click="gradeFilter = gradeFilter === g ? '' : g">
          {{ g }}
        </span>
      </div>
    ...
    <div class="ms__table-wrap">
      <table class="ms__tbl">
        <thead>
          <tr><th>时间</th><th>综合评分</th><th>健康等级</th><th>预测准确性</th><th>决策可靠性</th><th>物理合规性</th></tr>
        </thead>
        <tbody>
          <tr v-for="d in filteredScores" :key="d.time">
            <td>{{ d.time }}</td>
            <td>{{ (d.overall * 100).toFixed(0) }}</td>
            <td><ElTag :color="(HEALTH_GRADE as any)[d.grade]?.color" size="small" effect="dark">{{ d.grade }}</ElTag></td>
...
.ms {
  ...
  &__grades { display: flex; gap: 4px; }
  &__g { padding: 4px 12px; font-size: 13px; font-weight: 600; color: #64748b; background: #f1f5f9; border-radius: 4px; cursor: pointer;
    &.on { color: #fff; background: var(--gc, #3b82f6); }
  }
  ...
  &__table-wrap { flex-shrink: 0; max-height: 260px; overflow-y: auto; background: #fff; border-radius: 8px; border: 1px solid #eef0f2; }
  &__tbl { width: 100%; border-collapse: collapse;
    th { padding: 10px 12px; font-size: 13px; font-weight: 600; color: #64748b; text-align: left; border-bottom: 1px solid #eef0f2; position: sticky; top: 0; background: #fff; }
    td { padding: 9px 12px; font-size: 14px; border-bottom: 1px solid #f8fafc; }
