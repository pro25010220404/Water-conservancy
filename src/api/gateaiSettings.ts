...
export function fetchModelCompare(reservoirId: number, currentVer?: string, previousVer?: string) {
  ...
  return delay({
    current: {
      version: curOpt.version,
      source: curOpt.source,
      scores: versionScores(reservoirId, curOpt.version),
    },

    previous: {
      version: prevOpt.version,
      source: prevOpt.source,
      scores: versionScores(reservoirId, prevOpt.version),
    },
  })
}
...
export function savePhysicsGuardConfig(
...
export function fetchInterlockLogs(params?: {
  reservoirId?: number
  ruleCodes?: string[]
  startTime?: string
  endTime?: string
